...
namespace blyss
{
    StaticGeometryLoader::StaticGeometryLoader(std::shared_ptr<StaticShader> shader)
        : importer_{}
        , shader_{std::move(shader)}
    {
    }

    std::unique_ptr<StaticGeometry> StaticGeometryLoader::LoadGeometry(const std::string& path) const
    {
        // Create importer.
        ...
        return std::make_unique<StaticGeometry>(vertex_data, index_data, shader_);
    }
