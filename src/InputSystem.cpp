...
namespace blyss
{
    ...
    void InputSystem::GlfwKeyCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
    {
        ...
        Blyss* b = static_cast<Blyss*>(ptr);
        InputSystem& self = b->GetInput();
        ...
        if (key < 0 || s_key >= self.button_mapping_.size())
        {
        ...
        InputButton button = gsl::at(self.button_mapping_, s_key);
        ...
        assert(button_input_index > 0 && button_input_index < self.pressed_buttons_.size());

        // Update the button pressed state.
        if (action == GLFW_PRESS)
        {
            gsl::at(self.pressed_buttons_, button_input_index) = true;

            // Fire on key press event!
            KeyPressedEventArgs args{ button };
            KeyPressedEventHandler(*b, args);
        }
        else if (action == GLFW_RELEASE)
        {
            gsl::at(self.pressed_buttons_, button_input_index) = false;
        }
